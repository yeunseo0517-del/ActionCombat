...
전투 시스템은 공격 방식에 따라 역할을 분리하여 구성했습니다. 그 중 Weapon을 근접 Trace를 담당하는 객체로 정의했습니다.

- Weapon : 근접 Trace 공격
- Projectile : 투사체 공격
- Area Attack : 범위 공격

이 원칙에 따라 맨몸 공격과 무기가 Character Mesh에 포함된 경우도 **Weapon Mesh만 없는 근접 공격**라 판단해 별도의 파이프라인 없이 기존 Weapon 구조 안에서 처리하도록 구성했습니다.

근접 Trace가 필요한 객체라면 형태와 무관하게 Weapon으로 관리한다는 일관된 기준이 이후 설계 결정의 근거가 되었습니다.

본 문서에서는 맨몸 공격과 무기가 Character Mesh에 포함된 경우를 캐릭터 기반 공격으로 분류했습니다.
...
Unarmed Weapon처럼 여러 캐릭터가 공유하는 Weapon이 생기면서 "Trace Data를 누가 소유해야 하는가" 라는 문제가 생겼습니다.

실제로 플레이어 캐릭터의 맨손 공격은 Sphere Trace를 사용하지만 무기가 Character Mesh에 포함된 미니언 캐릭터는 Box Trace가 필요했습니다. Weapon에 Trace Data를 고정하면 특정 캐릭터의 Trace에 종속될 수 있었습니다.
...
또한 Weapon이 근접 Trace를 담당한다는 원칙과도 멀어지는 구조였습니다.

### 해결 - Override 구조

Weapon이 Trace Data를 소유한다는 원칙을 유지하면서 예외 상황에서만 Data Asset을 Override하는 방식을 채택했습니다.
